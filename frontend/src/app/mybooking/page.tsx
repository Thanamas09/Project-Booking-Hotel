...
export default function MyBookingPage() {
  ...
  const handleUpdate = async () => {
    if (session && editingBooking) {
      try {
        await dispatch(updateBookingThunk({ 
    ...
    <main style={{ backgroundColor: '#F9F6EE' }} className="min-h-screen py-20">
      <Container maxWidth="md">
        <Box className="text-center mb-20">
          ...
          <div style={{ backgroundColor: '#D4AF37' }} className="w-24 h-[1px] mx-auto mb-4 opacity-40"></div>
        ...
        {bookings.length > 0 ? (
            ...
            {bookings.map((b) => (
                ...
                <div className="text-center md:text-left flex-grow px-2">
                    ...
                    <Typography style={{ color: '#D4AF37' }} className="font-medium text-xs uppercase tracking-[0.2em] mb-4 block">
                    {b.hotel?.province}
                    </Typography>
                    ...
                    <div className="flex flex-col">
                        <span className="text-[10px] text-gray-400 uppercase tracking-widest mb-1">Check-in</span>
                        <b className="text-gray-700 text-base">{new Date(b.checkinDate).toLocaleDateString('en-GB')}</b>
                    </div>
                    <div className="flex flex-col">
                        <span className="text-[10px] text-gray-400 uppercase tracking-widest mb-1">Check-out</span>
                        <b className="text-gray-700 text-base">{new Date(b.checkoutDate).toLocaleDateString('en-GB')}</b>
                    </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-3 w-full md:w-48 shrink-0">
                    ...
                    sx={{ borderColor: '#D4AF37', color: '#D4AF37', borderRadius: '50px', py: 1.2, fontWeight: 'bold' }}
                    >
                    Edit Booking
                    </Button>
                    <Button 
                    ...
                    sx={{ bgcolor: '#FEF2F2', color: '#DC2626', borderRadius: '50px', py: 1.2, boxShadow: 'none', fontWeight: 'bold' }}
                    >
                    Cancel
                    </Button>
                </div>
            ...
            <Typography className="font-serif italic text-gray-400">No active reservations.</Typography>
            </Box>
        )}
        </div>
